...
def train(D, G, x, y, adj, device):
    x, y, adj = torch.tensor(x).to(device), torch.tensor(y).to(device), torch.tensor(adj).to(device)
    #dataset = Data.TensorDataset(x, y, adj)
    #train_loader = Data.DataLoader(dataset=dataset, batch_size=opt.batch_size, shuffle=True)
    dataset = torch.utils.data.TensorDataset(x, y, adj)
    train_loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=opt.batch_size) # no shuffle here.
    ...
    for epoch in range(opt.epoch):
        print('this is epoch', epoch, time.ctime())
        D_losses = []
        G_losses = []
    ...
    show_train_hist(train_hist, path=f'{path}/train_loss_hist.png')
...
def main():
    ...
    D = Discriminator(opt.D_input_feat, opt.D_hidden_feat, opt.D_final_feat, opt.num_head, opt.dropout, opt.num_block, opt.num_variable, opt.seq_len).to(device)
    ...
    start = time.ctime()
    train(D, G, train_seq_x, train_seq_y, train_seq_adj, device)
    end = time.ctime()
