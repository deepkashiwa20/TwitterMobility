...
class Building_Block(nn.Module):
    def __init__(self, input_features, hidden_features, num_heads, dropout, num_variable):
        ...
        self.embed_dim = num_variable * self.hidden_features
...
class Generator(nn.Module):
    ...
    def __init__(self, input_features, hidden_features, final_features, num_heads, dropout, num_block, num_variable):
        ...
        self.blocks = get_clones(Building_Block(1, 1, num_heads, dropout, num_variable), self.num_block)
...
### Global Variable, used by BuildingBlock ###
...
def main():
    ...
    D_x_init_features = 1
    G_x_init_features = 100
    D_hidden_feat = 16
    G_hidden_feat = 64
    D_input_feat = D_x_init_features + num_condition
    G_input_feat = G_x_init_features + num_condition
    
    D = Discriminator(D_input_feat, D_hidden_feat, final_feat, num_head, dropout, num_block, num_variable).to(device)
    G = Generator(G_input_feat, G_hidden_feat, final_feat, num_head, dropout, num_block, num_variable).to(device)
    adj = (sequence_length, num_variable, num_variable)
    c = (sequence_length, num_variable, num_condition)
    D_x = (sequence_length, num_variable, D_x_init_features)
    G_x = (sequence_length, num_variable, G_x_init_features)
